...
/**
 * Class that represents filled circle that the user draws. The first click
 * defines the filled circle center and as user moves the mouse, a circle radius
 * is defined. On second click, filled circle is added.
 * 
 * @author tina
 *
 */
public class FilledCircle extends AbstractGeometricalObject {
	/**
	...
	/**
	 * Pattern: FCIRCLE centerx centery radius red green blue red green blue
	 */
	@Override
	public String formatOutput() {
		return String.format("FCIRCLE %d %d %d %d %d %d %d %d %d", startPoint.x, startPoint.y, radius, fgColor.getRed(),
	...
	public void draw(Graphics2D g, int upperX, int upperY) {
		...
		g.setColor(bgColor);
		g.fillOval(x - upperX, y - upperY, 2 * radius, 2 * radius);
	}
	...
	private void addLabels(List<JLabel> labels) {
		...
		labels.add(new JLabel("Background color red"));
		labels.add(new JLabel("Background color green"));
		labels.add(new JLabel("Background color blue"));
	}
	...
	private void addInputs(List<JTextField> inputs) {
		...
		field = new JTextField();
		field.setText(String.valueOf(bgColor.getRed()));
		inputs.add(field);

		field = new JTextField();
		field.setText(String.valueOf(bgColor.getGreen()));
		inputs.add(field);

		field = new JTextField();
		field.setText(String.valueOf(bgColor.getBlue()));
		inputs.add(field);
	...
	private void setParameters(List<JTextField> inputs) {
		...
		bgColor = new Color((int) Double.parseDouble(inputs.get(6).getText()),
				(int) Double.parseDouble(inputs.get(7).getText()), (int) Double.parseDouble(inputs.get(8).getText()));
		notifyListeners();
	}
}
