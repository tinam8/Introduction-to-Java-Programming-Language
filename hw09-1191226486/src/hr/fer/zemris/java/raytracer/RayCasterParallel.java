...
/**
 * Parallelized program that creates image based on objects and light sources
 * positions in the scene. Geometric rays are traced from the eye of the
 * observer to sample the light (radiance) traveling towards the observer from
 * the ray direction.
 * 
 * @author tina
 *
...
public class RayCasterParallel {
	...
	/**
	 * Class that implements work for single thread and if condition is not
	 * satisfied creates two threads recursively.
	 * 
	 * @author tina
	 *
	 */
	public static class ComputingWork extends RecursiveAction {
		/**
		 * Version
		 */
		private static final long serialVersionUID = 1L;
		/** horizontal size of plane */
		double horizontal;
		/** vertical size of plane */
		double vertical;
		/** screen width */
		int width;
		/** screen height */
		...
		short[] blue;
		/** minimum height per block */
		static final int treshold = 16;

		/**
		 * Constructor that sets variables of the class.
		 * 
		 * @param horizontal
		 *            horizontal size of plane
		 * @param vertical
		 ...
		 *            minimum height per block
		 */
		public ComputingWork(double horizontal, double vertical, int width, int height, int yMin, int yMax, Point3D eye,
				Point3D screenCorner, Point3D xAxis, Point3D yAxis, Scene scene, short[] red, short[] green,
				short[] blue) {
		...
		public void compute() {
			...
			invokeAll(
					new ComputingWork(horizontal, vertical, width, height, yMin, yMin + (yMax - yMin) / 2, eye,
							screenCorner, xAxis, yAxis, scene, red, green, blue),
					new ComputingWork(horizontal, vertical, width, height, yMin + (yMax - yMin) / 2 + 1, yMax, eye,
							screenCorner, xAxis, yAxis, scene, red, green, blue));
		}

		/**
		 * Method that calculate s color for image for certain segment.
		 */
	...
	private static IRayTracerProducer getIRayTracerProducer() {
		...
		return new IRayTracerProducer() {
			...
			public void produce(Point3D eye, Point3D view, Point3D viewUp, double horizontal, double vertical,
				...
				// novi posao
				pool.invoke(new ComputingWork(horizontal, vertical, width, height, 0, height - 1, eye, screenCorner,
						xAxis, yAxis, scene, red, green, blue));
				// imam rezultate
				pool.shutdown();
